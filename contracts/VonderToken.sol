...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    function _transferWithLock(
        ...
        _rewardPerBlock[recipient] = _balancesLock[recipient].mul(10000).div(
            _totalBlockRelease
        );
    ...
    function getRewardLockToClaim(
        ...
        if (block.number > _endClaimBlock[account]) {
            ...
            return
                (block.number.sub(_lastClaimBlock[account]))
                    .mul(_rewardPerBlock[account])
                    .div(10000);
        }
    }
