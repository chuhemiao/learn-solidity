...
contract MasterChef is Ownable {
    ...
    function pendingVon(
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 vonReward = multiplier
                .mul(vonPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accVonPerShare = accVonPerShare.add(
                vonReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accVonPerShare = pool.accVonPerShare.add(
            vonReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accVonPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeVonTransfer(msg.sender, pending);
    ...
    function setVonTotalBlockRelease(
        uint256 totalBlockRelease
    ) public onlyOwner {
